...
std::vector<std::array<int, 2>> findPath(int idx_x, int idx_y, int goal_x, int goal_y)
{
  ...
  {
    // ---------------------- (1) Poll the queue -------------------------
    Node *cur_node = queue.front(); // WHAT DOES THIS LINE OF CODE DO? Sets the head of the queue as the current node the program is visiting at by having the pointer variable "cur_node" point to the node at the head of the queue.
    queue.pop_front();              // WHAT DOES THIS LINE OF CODE DO? Pops/Removes the node at the head of the queue. 
    int cur_x = cur_node->x;        // WHAT DOES THIS LINE OF CODE DO? Assigns the variable "cur_x" with the x-coordinate of the current node.
    int cur_y = cur_node->y;        // WHAT DOES THIS LINE OF CODE DO? Assigns the variable "cur_y" with the y-coordinate of the current node.
    int cur_cost = cur_node->cost;

    // ---------------------- (2) Find the path if goal is found, and break loop -------------------------
    if (cur_x == goal_x && cur_y == goal_y)   // WHAT DOES THIS LINE OF CODE DO? Check if the current node is the destination by checking if the coordinates of the current node match the destination coordinates.
    {
      Node *node = cur_node;
      do
      {
        path.push_back({node->x, node->y}); // push back current node's x and y
        node = node->parent;                // move to parent
      } while (node != nullptr);            // until we reach the start
      break;
    }
